...
def prepare_pdbs_from_dir(pdb_dir, pdb_output_dir):
    """
    Copy all PDB files from pdb_dir to pdb_output_dir with names stem_0.pdb
    so the dataloader gets one structure per file (no CSV needed).
    """
    os.makedirs(pdb_output_dir, exist_ok=True)
    pdbs = sorted(glob.glob(os.path.join(pdb_dir, "*.pdb")))
    if not pdbs:
        raise FileNotFoundError(f"No .pdb files found in {pdb_dir}")
    for src in tqdm(pdbs, desc="Preparing PDBs"):
        stem = os.path.splitext(os.path.basename(src))[0]
        dst = os.path.join(pdb_output_dir, f"{stem}_0.pdb")
        shutil.copy2(src, dst)
    print(f"✅ Prepared {len(pdbs)} PDBs in {pdb_output_dir}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--csv_file', type=str, default=None,
                        help='Optional CSV with link_name, seq, seq_idx. If omitted, all .pdb files in pdb_dir are used as-is.')
    ...
    if args.csv_file:
        # Merge structure + sequence from CSV
